...
module.exports = {
  ...
  pageProducts: page => {
    return new Promise((resolve, reject) => {
      ...
      connection.query(
        `SELECT products.*, categories.name as categories FROM products INNER JOIN categories ON products.category_id = categories.id WHERE deleted = 0 LIMIT ${total}, ${display}`,
        (err, result) => {
          if (!err) {
            resolve(result);
          } else {
